...
class MessageParserTest {
    ...
    @Test
    fun `should parse system message with continuation lines (Meta AI notice format)`() {
        val line =
            """
            11/20/24, 13:55 - Only messages that mention or people share with @AI can be read by Meta. Meta can't read any other messages in this chat, as your personal messages remain end-to-end encrypted.

            Messages are generated by AI. Some may be inaccurate or inappropriate. *Learn more*
            """.trimIndent()

        val message = MessageParser().parse(line) { it }
        assertEquals(LocalDateTime.of(2024, 11, 20, 13, 55), message.createdAt)
        assertEquals(AuthorType.SYSTEM, message.author.type)
        assertEquals("", message.author.name)
    }

    @Test
    fun `should fall back to auto-detect when custom formatter cannot parse the date`() {
        // Custom formatter is dd/MM/yyyy HH:mm (BR format), but file has US format MM/dd/yy
        MessageParser("dd/MM/yyyy HH:mm").apply {
            assertEquals(LocalDateTime.of(2023, 12, 24, 18, 15), this.parseDate("12/24/23, 18:15"))
            assertEquals(LocalDateTime.of(2023, 11, 16, 18, 44), this.parseDate("11/16/23, 18:44"))
        }
    }
